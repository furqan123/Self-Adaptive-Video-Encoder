...
class MPCController:
	...
	def __init__(self,A,B,C,D,L,Q,P,Lk,Pk,Qn,Rn,Umin,Umax,DeltaUmin,DeltaUmax,optim=True,fast=False,time_varying=False):
		...
		self.n = A.shape[0] # 1 A matrix has 1 element
		self.m = B.shape[1] # 3 B matrix has 3 elements
		self.p = C.shape[0] # 2 C matrix has 2 elements
		#print "n"
		#print self.n
		#print "m"
		#print self.m
		...
		# np.zeros((self.n,self.p)) get number of zeros according to the value of p
		# A is 0.968022
		self.At = np.r_[np.c_[A,np.zeros((self.n,self.p))],np.c_[C,np.eye(self.p)]] # merge tables _c does that and r_
		#print "At"
		#print self.At
		...
		self.nt = self.At.shape[0] #shape[0] returns number of rows and shape[1] number of columns
		...
		if fast and not(optim):
			...
			self.Q  = Q
			self.P  = P # r
			self.P1 = P[0:self.m,0:self.m] # 3 by 3 matrix of P 
			#print self.L
			#print self.Q
			#print self.P
			#print self.P1
		...
		# Controller state
		self.uu = np.zeros((self.m,1)) # cloumn 1 and rows 3 m=3 fill with zeros
		#print "uu"
		#print self.uu
	...
	def compute_optim_matrices(self,At,Bt,Ct,L,Q,P):
		...
		# Control matrices
		OL = np.zeros((L*pt,nt)); # l=4*pt=2,nt=3
		#print OL
		for i in range(1,L+1):
		    OL[(i-1)*pt:i*pt,:] = Ct*np.linalg.matrix_power(At,i-1); #
		    #print Ct*np.linalg.matrix_power(At,i-1)
